...
def installation_status(root: Path) -> dict:
    ...
    keys = ('agent_server', 'automation_api', 'session_key_file', 'automation_key_file', 'canvas_base')
    return {'version': VERSION, 'data_dir': str(root),
            'installed': bool(read_json(root / 'current.json', {})),
            ...
            'integration': {key: integration[key] for key in keys if key in integration},
...
def probe(root: Path) -> dict:
    ...
    return {**installation_status(root), 'running': bool(state), 'runtime': detail}
